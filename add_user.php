...
$username = $email = $company = $password = $confirm_password = $role = $status = "";
$success = "";
$error = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO adminlist (username, email, company, password, role, status) VALUES (:username, :email, :company, :password, :role, :status)");
        ...
        if ($stmt->execute()) {
            ...
            $username = $email = $company = $password = $confirm_password = "";
            $role = "User";
            $status = "Active";
        }
        // Fail silently if insert fails
    }
}
?>
  ...
  <div class="card card-lightblue p-4 rounded-4">
    ...
    <form class="row g-3" method="POST" action="">
      <div class="col-md-4">
        ...
        <input type="text" name="username" class="form-control" placeholder="Enter username" value="<?= htmlspecialchars($username) ?>" required>
      </div>
      ...
      <div class="col-md-4">
        ...
        <input type="text" name="company" class="form-control" placeholder="Enter company name" value="<?= htmlspecialchars($company) ?>">
      </div>
